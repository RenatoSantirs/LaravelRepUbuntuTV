<x-app-layout>
    ...
    <div class="p-6 flex flex-col gap-6 overflow-hidden bg-white rounded-md shadow-md lg:flex-row md:justify-between dark:bg-dark-eval-1">
        <div>
            <h3 class="text-lg font-semibold mb-4">Categorías Disponibles</h3>
            <div class="grid grid-cols-2 gap-4">
                @foreach($categorias as $categoria)
                <a href="{{ route('articulo-categorias', $categoria) }}" class="text-blue-600 hover:underline">{{ $categoria }}</a>
                @endforeach
            </div>
        </div>
